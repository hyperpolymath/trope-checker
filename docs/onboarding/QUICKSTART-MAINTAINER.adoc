...
// Replace rsr-template-repo, trope-checker, {{DEPS}} with actuals
...
| `$PREFIX/share/trope-checker/`
| Data files, assets

| `$PREFIX/share/doc/trope-checker/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/trope-checker/config.toml`

Fallback: `$HOME/.config/trope-checker/config.toml`
...
Or via OPSM: `opsm update trope-checker`
...
[source,bash]
----
just install --prefix=/opt/trope-checker-instance1 --config=/etc/trope-checker/instance1.toml
just install --prefix=/opt/trope-checker-instance2 --config=/etc/trope-checker/instance2.toml
----
