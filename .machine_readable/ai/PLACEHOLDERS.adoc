...
sed -i 's/Trope Checker/my-project/g' $(grep -rl 'Trope Checker' .)
sed -i 's/TROPE_CHECKER/MY_PROJECT/g' $(grep -rl 'TROPE_CHECKER' .)
sed -i 's/trope_checker/my_project/g' $(grep -rl 'trope_checker' .)
sed -i 's/trope-checker/my-project/g' $(grep -rl 'trope-checker' .)
sed -i 's/github.com/github.com/g' $(grep -rl 'github.com' .)
sed -i "s/2026/$(date +%Y)/g" $(grep -rl '2026' .)
sed -i "s/2026-06-22/$(date +%Y-%m-%d)/g" $(grep -rl '2026-06-22' .)
```
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Trope Checker` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `The portable trust boundary of the trope-particularity calculus: a pure function from a language-neutral Trope IR to a sufficiency verdict.` | One-line description | `A tool for X` | flake.nix |
| `TROPE_CHECKER` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `trope_checker` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `trope-checker` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `2026-06-22` | Current date (ISO) | `2026-02-14` | STATE.a2ml, MAINTAINERS.md |
| `2026-06-22` | Last updated date | `2026-02-14` | TOPOLOGY.md, THREAT-MODEL.md |
...
| `https://github.com/hyperpolymath/trope-checker` | Project website | `https://example.org` | SECURITY.md |
...
| `The portable trust boundary of the trope-particularity calculus: a pure function from a language-neutral Trope IR to a sufficiency verdict.` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
