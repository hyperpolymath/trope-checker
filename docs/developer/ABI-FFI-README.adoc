...
# TROPE_CHECKER ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libtrope_checker.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── trope_checker.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── trope_checker.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/trope_checker.h
```
...
```c
#include "trope_checker.h"

int main() {
    void* handle = trope_checker_init();
    if (!handle) return 1;

    int result = trope_checker_process(handle, 42);
    if (result != 0) {
        const char* err = trope_checker_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    trope_checker_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -ltrope_checker -L./zig-out/lib
```

### From Idris2

```idris
import TROPE_CHECKER.ABI.Foreign
...
```rust
#[link(name = "trope_checker")]
extern "C" {
    fn trope_checker_init() -> *mut std::ffi::c_void;
    fn trope_checker_free(handle: *mut std::ffi::c_void);
    fn trope_checker_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = trope_checker_init();
        assert!(!handle.is_null());

        let result = trope_checker_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libtrope_checker = "libtrope_checker"

function init()
    handle = ccall((:trope_checker_init, libtrope_checker), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:trope_checker_process, libtrope_checker), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:trope_checker_free, libtrope_checker), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/trope_checker.h
   ```
